...
#Все цели обращения (appointment/goals/list) ->
  ...
  Сценарий: Отмена записи (предварительный просмотр слота для записи, ее создание и отмена)
    Дано тело запроса:
      """
      {
        "patientId": "string",
  ...
  #
  # Попробовать доделать 400 код ответа --------------------------------------------------------------
  #
  ...
  #
  # Спросить по поводу данных, которые можно использовать внутри запроса (нет понимая, где брать referralId и пр.
  #

  #Данный сценарий перекрывается предыдущим кейсом на удаление записи, вопрос, есть ли необходимость в отдельном
  #полноценном сценарии
  ...
  # Запрос на получение записи (создать и вытащить id записи) appointments/list, appointments/get
  #
  #
